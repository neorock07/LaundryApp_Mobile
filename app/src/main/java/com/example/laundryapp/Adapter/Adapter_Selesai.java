...
public class Adapter_Selesai extends RecyclerView.Adapter<Adapter_Selesai.MyViewHolder> {
    Context context;
    List<Model_Data> list_data;
     public Adapter_Selesai(Context context, List<Model_Data> list_data){
    ...
    @NonNull
    @Override
    public Adapter_Selesai.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.container_selesai,parent,false);
         return new MyViewHolder(v);
    }

    @SuppressLint("ResourceAsColor")
    @Override
    public void onBindViewHolder(@NonNull Adapter_Selesai.MyViewHolder holder, int position) {
        try{
            Model_Data model = list_data.get(position);
            holder.tx_berat.setText(model.getBerat() + "Kg");
            holder.tx_dead.setText( "Selesai pada, "+ FormatTime(model.getDeadline()));
            holder.tx_masuk.setText(FormatTime(model.getTanggal()));
            ...
            holder.ambil.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    AlertDialog alert = new AlertDialog.Builder(context)
                            .setMessage("Data akan dipindahkan ke label Diambil, Pastikan cucian sudah diambil Pelanggan ya!")
                            .setIcon(R.drawable.logo)
                            .setTitle("Konfirmasi")
                            .setPositiveButton("Diambil", (dialogInterface, i) -> {
                                try{
                                    UpdateAmbil(Integer.parseInt(holder.tx_no.getText().toString()));
                                }catch (Exception e){
    ...
    private void UpdateAmbil(int id){
        ApiRequestData data = Koneksi_Server.koneksi().create(ApiRequestData.class);
        Call<Model_Response> respon = data.UpdateAmbil(id);
        respon.enqueue(new Callback<Model_Response>() {
            ...
            public void onResponse(Call<Model_Response> call, Response<Model_Response> response) {
                Toast.makeText(context, "Memindahkan data ke Diambil", Toast.LENGTH_SHORT).show();
            }
        ...
        Button ambil;
         public MyViewHolder(@NonNull View itemView) {
            super(itemView);
             ...
             ambil = itemView.findViewById(R.id.btn_proses);
