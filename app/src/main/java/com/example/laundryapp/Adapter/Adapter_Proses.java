...
public class Adapter_Proses extends RecyclerView.Adapter<Adapter_Proses.MyViewHolder> {
    Context context;
    List<Model_Data> dataList;

    public Adapter_Proses(Context context, List<Model_Data> dataList){
    ...
    @NonNull
    @Override
    public Adapter_Proses.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v  = LayoutInflater.from(context).inflate(R.layout.container_proses, parent,false);
        return new MyViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull Adapter_Proses.MyViewHolder holder, int position) {
        try{
            Model_Data model = dataList.get(position);
            holder.tx_berat.setText(model.getBerat() + "Kg");
            holder.tx_dead.setText(model.getDeadline());
