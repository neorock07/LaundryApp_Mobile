...
public class Adapter_Ambil extends RecyclerView.Adapter<Adapter_Ambil.MyViewHolder> {
    Context context;
    List<Model_Data> daftar;

    public Adapter_Ambil(Context context, List<Model_Data> daftar){
    ...
    @NonNull
    @Override
    public Adapter_Ambil.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.container_ambil, parent, false);
        return new MyViewHolder(v);
    }

    @SuppressLint("ResourceAsColor")
    @Override
    public void onBindViewHolder(@NonNull Adapter_Ambil.MyViewHolder holder, int position) {
        try{
            Model_Data model = daftar.get(position);
            holder.tx_berat.setText(model.getBerat() + "Kg");
            holder.tx_dead.setText("Selesai pada, " + FormatTime(model.getTanggal()));
